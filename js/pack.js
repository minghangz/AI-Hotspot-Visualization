...
function drawMatrix(d){
    ...
    edges.forEach(function(link) {
        matrix[link.source][link.target].z += link.val;
        matrix[link.target][link.source].z += link.val;
        matrix[link.source][link.source].z += link.val;
        matrix[link.target][link.target].z += link.val;
    });
    ...
    z.domain([0, d3.max(edges.map(d=>d['val']))]).clamp(true);
